...
public class TestEnsembleVoituresAutorisees{
	...
	public static void main(String[] args) {	
		...
		if(e.retirerVoiture(new Voiture("BBB000",""))){
			System.out.println("la voiture a déjà été retirée petit pd");
			return;
		}
		if(e.retirerVoiture(new Voiture("BBB000",""))){
			System.out.println("la voiture a déjà été retirée petit pd");
		...
		if(! e.voitureAutorisee(new Voiture("AAA000",""))){
			System.out.println("Attention, votre methode annonce que la voiture AAA000 est non autorisee !");
			return;
		}
		if(! e.voitureAutorisee(new Voiture("CCC000",""))){
			System.out.println("Attention, votre methode annonce que la voiture CCC000 est non autorisee !");
			return;
		}

		System.out.println("Cas 7 : ajout de la voiture 1AAA000 (qui y est deja!)");
		if(e.ajouterVoiture(new Voiture("AAA000",""))){
			System.out.println("Attention la voiture AAA000 est déjà présente mas vous l'avez quand même ajoutée");
			return;
		}
	}
}